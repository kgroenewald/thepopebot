...
export async function initiateOAuthFlow({ secretName, clientId, clientSecret, tokenUrl, scopes, secretType, returnPath }) {
  ...
  try {
    ...
    const state = createOAuthState({ secretName, clientId, clientSecret, tokenUrl, secretType: secretType || 'agent_job_secret', returnPath: returnPath || '/admin/event-handler/jobs' });
